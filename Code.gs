...
const ANSWERS = {1:'A',2:'B',3:'B',4:'D',5:'A',6:'C',7:'B',8:'D'};
...
function setup() {
  ...
  const headers = ['Data/hora','Data de realização','Turma','Nome','RA','Nº de chamada','E-mail institucional','Q1','Q2','Q3','Q4','Q5','Q6','Q7','Q8','Q9 — dissertativa','Q10 — dissertativa','Gabarito objetivos','Q1–Q8 — resultado','Acertos objetivos','Nota inteira','Percentual','AE5 — status','Saúde e saneamento — status','AEs não atingidas'];
  ...
  const correct = SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=REGEXMATCH($V2,"CORRETA")').setBackground('#2f75b5').setFontColor('#ffffff').setRanges([sh.getRange('H2:O')]).build();
  const wrong = SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=REGEXMATCH($V2,"ERRADA")').setBackground('#c00000').setFontColor('#ffffff').setRanges([sh.getRange('H2:O')]).build();
...
function submitAnswers(p) {
  ...
  for(let i=1;i<=8;i++){const v=String(a[i]||'').toUpperCase(); answers.push(v); if(v===ANSWERS[i]) acertos++;}
  const status=answers.map((v,i)=>v===ANSWERS[i+1]?'CORRETA':'ERRADA');
  const nota=Math.round((acertos/8)*10), percentual=Math.round((acertos/8)*100);
  const ae5Ok=answers.slice(0,6).filter((v,i)=>v===ANSWERS[i+1]).length>=4, saudeOk=answers.slice(6,8).filter((v,i)=>v===ANSWERS[i+7]).length>=1;
  ...
  const row=[new Date(),p.dataRealizacao||'',p.turma,p.nome,p.ra,p.chamada||'',p.email||'',...answers,a[9]||'',a[10]||'',Object.values(ANSWERS).join(' | '),status.join(' | '),acertos,nota,percentual+'%',ae5Ok?'ATINGIDA':'NÃO ATINGIDA',saudeOk?'ATINGIDA':'NÃO ATINGIDA',nao.join(' | ')||'Nenhuma'];
